...
namespace Evade
{
    static class Extensions
    {
        public static List<Vector2> GetWaypoints(this Obj_AI_Base unit)
        {
            ...
            if (unit.IsVisible)
            {
                result.Add(EloBuddy.SDK.Extensions.To2D(unit.ServerPosition));
                var path = unit.Path;
                if (path.Length > 0)
                {
                    var first = EloBuddy.SDK.Extensions.To2D(path[0]);
                    ...
                    {
                        result.Add(EloBuddy.SDK.Extensions.To2D(path[i]));
                    }
                }
            }
        ...
        public static List<Vector2> CutPath(this List<Vector2> path, float distance)
        {
            ...
            if (distance < 0)
            {
                path[0] = path[0] + distance * EloBuddy.SDK.Extensions.Normalized((path[1] - path[0]));
                return path;
            }
            ...
            for (var i = 0; i < path.Count - 1; i++)
            {
                ...
                if (dist > Distance)
                {
                    result.Add(path[i] + Distance * EloBuddy.SDK.Extensions.Normalized((path[i + 1] - path[i])));
        ...
        public static bool InAutoAttackRange(AttackableUnit target)
        {
            ...
            return
                Vector2.DistanceSquared(
                    target is Obj_AI_Base ? EloBuddy.SDK.Extensions.To2D(((Obj_AI_Base)target).ServerPosition) : EloBuddy.SDK.Extensions.To2D(target.Position),
                    EloBuddy.SDK.Extensions.To2D(Player.ServerPosition)) <= myRange * myRange;
        }
    ...
    public static class Dash
    {
        ...
        private static void ObjAiHeroOnOnNewPath(Obj_AI_Base sender, GameObjectNewPathEventArgs args)
        {
            if (sender.IsValid() && sender.IsVisible && sender.IsHPBarRendered)
            {
                ...
                if (args.IsDash)
                {
                    var path = new List<Vector2> { EloBuddy.SDK.Extensions.To2D(sender.ServerPosition) };
                    ...
                    DetectedDashes[sender.NetworkId].StartPos = EloBuddy.SDK.Extensions.To2D(sender.ServerPosition);
